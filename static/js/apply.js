...
function addMemberRow() {
    ...
    div.innerHTML = `
        <div class="flex items-center justify-between pb-2 border-b border-slate-200/60">
            ...
            <button type="button" onclick="removeMemberRow('${rowId}')" class="text-xs text-rose-500 hover:text-rose-700 font-medium flex items-center gap-1">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
                ...
                <input type="text" class="member-first-name mt-0.5 block w-full px-2.5 py-1.5 border border-slate-300 rounded-lg text-xs" required placeholder="First name">
            </div>
            <div>
                <label class="block text-[11px] font-semibold text-slate-600">Last Name</label>
                <input type="text" class="member-last-name mt-0.5 block w-full px-2.5 py-1.5 border border-slate-300 rounded-lg text-xs" required placeholder="Last name">
            </div>
            <div>
                <label class="block text-[11px] font-semibold text-slate-600">Relationship to Head</label>
                <select class="member-rel mt-0.5 block w-full px-2.5 py-1.5 border border-slate-300 rounded-lg text-xs bg-white">
...
async function handleApplicationSubmit(e) {
    ...
    try {
        ...
        if (!res.ok) {
            throw new Error(data.detail || 'Application failed to submit.');
        }
...
function copyReferenceNumber() {
    ...
    navigator.clipboard.writeText(ref).then(() => {
        alert(`Reference number ${ref} copied to clipboard!`);
    }).catch(() => {
